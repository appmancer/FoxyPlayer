...
class RealPCloudAPITest {
            ...
            if (realAuthResult.isSuccess) {
                ...
                if (realApiResult.isSuccess) {
                    ...
                    if (response.contains("\"contents\"")) {
                        ...
                        if (!foundContents) {
                            println("  📋 Response contains contents but format differs from expected")
                        }
                ...
                val autoServerResult = authRepository.authenticateWithAutoServerDetection(email, password)
                if (autoServerResult.isSuccess) {
                    ...
                    println("❌ Auto server detection failed: ${autoServerResult.exceptionOrNull()?.message}")
                }
                ...
                if (musicResult.isSuccess) {
                    ...
                    println("🔗 Uses real auth token: ${musicResponse.containsAuthToken(authResponse.authToken)}")
                ...
                println("❌ REAL Authentication failed: ${realAuthResult.exceptionOrNull()?.message}")
                ...
                when {
                    ...
                    exception?.message?.contains("SSL") == true -> {
                        println("💡 This appears to be an SSL/TLS error - check network configuration")
                    }
                    else -> {
    ...
    fun `test real pCloud response parsing`() {
        ...
        assertEquals("Should extract correct auth token", "DOtgukZVnEQZ54VYwK4DE4Bwgc4lJaoDxkLyx17V", authResponse.authToken)
        ...
        assertTrue("Response should contain real user data", realPCloudResponse.contains("\"premium\": true"))
