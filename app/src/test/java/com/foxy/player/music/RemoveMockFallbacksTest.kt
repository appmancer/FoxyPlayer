...
class RemoveMockFallbacksTest {
    ...
    fun `listAudioFilesWithPagination should not return hardcoded page1_song1_mp3 patterns`() {
        ...
        if (result.isSuccess) {
            ...
            val hasMockPattern = paginatedResponse.audioFiles.any { it.startsWith("page1_") || it.startsWith("page2_") }
    ...
    fun `discoverAlbums should not return hardcoded Artist_Album_Song patterns`() {
        ...
        if (result.isSuccess) {
            ...
            val hasMockAlbumPattern = allAudioFiles.any { it.matches(Regex("Artist - Album - Song\\d+\\.mp3")) }
    ...
    fun `listAudioFilesWithErrorHandling should not return hardcoded retry patterns when retryScenario is false`() {
        ...
        if (result.isSuccess) {
            ...
            val hasRetryPattern = audioFiles.any { it.contains("_retry1.mp3") || it.contains("_retry2.flac") }
            ...
            val hasDefaultPattern = audioFiles.any { it.contains("_default1.mp3") || it.contains("_default2.flac") }
