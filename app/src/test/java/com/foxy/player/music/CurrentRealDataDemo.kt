...
class CurrentRealDataDemo {
    ...
    fun `demonstrate real pCloud data parsing`() {
        ...
        assertEquals("Real token should match", "DOtgukZVnEQZ54VYwK4DE4Bwgc4lJaoDxkLyx17V", authResponse.authToken)
        
        // Show real account details from response
        assertTrue("Should contain real premium status", realPCloudResponse.contains("\"premium\": true"))
        assertTrue("Should contain real quota", realPCloudResponse.contains("\"quota\": 536870912000"))
    ...
    @Test  
    fun `show what real folder listing would look like`() {
